      ...
      if (error) {
        ...
        if (error.message?.includes('stripe_not_configured')) {
          toast.info('Billing portal coming soon', {
            description:
              'Stripe integration is not yet configured. This feature will be available after setup.',
          });
                  ...
                  <div className="border border-border rounded-lg p-5 max-w-lg mb-5">
                    <div className="flex items-start justify-between mb-4">
                      <div>
                        ...
                        {!isPro && !isProPlus && (
                          <p className="text-xs text-muted-foreground mt-1">Free — no expiry</p>
                        )}
                      </div>
                ...
                <div className="max-w-lg p-4 rounded-md bg-muted/40 border border-border">
                  <p className="text-xs text-muted-foreground">
                    <span className="font-medium text-foreground">
                      Billing is managed by Stripe.
                    </span>{' '}
                    Colabs never stores your card details. All payment information is securely
                    handled by Stripe's PCI-compliant infrastructure. Cancellations take effect at
                    the end of your current billing period.
                    {/* TODO: Remove this note once Stripe is configured */} Stripe integration is
                    currently being set up — billing management will be fully available soon.
                  </p>
                </div>
              </div>
            )}
                    ...
                    {[
                      ...
                      <div key={svc.name} className="flex items-center justify-between px-4 py-3">
                        <div className="flex items-center gap-3">
                          <div
                            className={`h-7 w-7 ${svc.bg} rounded flex items-center justify-center`}
                          >
