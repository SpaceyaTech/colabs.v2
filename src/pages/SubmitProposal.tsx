...
import {
  Plus, Trash2, ArrowLeft, DollarSign, Clock, FileText, Github,
  Globe, Send, GripVertical, AlertCircle, CheckCircle2, Briefcase,
...
const SubmitProposal = () => {
  ...
  useEffect(() => {
    ...
    meta.content = `Submit your proposal for a gig on Colabs — the developer collaboration platform.`.slice(0, 155);
    let canonical = document.querySelector('link[rel="canonical"]') as HTMLLinkElement | null;
    if (!canonical) { canonical = document.createElement("link"); canonical.rel = "canonical"; document.head.appendChild(canonical); }
    canonical.href = canonicalHref;
  }, [projectId, canonicalHref]);

  const handleAddMilestone = () => setMilestones(m => [...m, { title: "", description: "", duration: "", amount: "" }]);
    ...
    try {
      ...
      if (resumeFile) {
        ...
        if (!allowedExts.includes(fileExt)) {
          toast({ title: "Invalid file extension", description: "Allowed: PDF, DOC, DOCX.", variant: "destructive" });
          setSubmitting(false);
          return;
        }
      ...
      if (paymentType === "fixed") {
        ...
        if (cleaned.length > 0) {
          const { error: msError } = await supabase.from("proposal_milestones" as any).insert(cleaned);
          if (msError) throw msError;
        }
      }

      toast({ title: "Proposal submitted!", description: "You'll be notified when the client responds." });
      navigate("/proposals");
    } catch (err: any) {
        ...
        <div className="max-w-3xl mx-auto px-4 sm:px-6 py-6 sm:py-8">
          ...
          <div className="space-y-6">
            ...
            <section className="space-y-4">
              ...
              <p className="text-[11px] text-muted-foreground -mt-2">Explain how you'd tackle this gig and why you're the right fit</p>
              ...
              <div className="space-y-1.5">
                ...
                <Textarea
                  ...
                  maxLength={2000}
                />
                <p className="text-[10px] text-muted-foreground text-right">{coverLetter.length}/2000</p>
              </div>
            ...
            <section className="space-y-4">
              ...
              {paymentType === "hourly" ? (
                <div className="grid sm:grid-cols-2 gap-4">
                  ...
                  <div className="space-y-1.5">
                    <Label htmlFor="estimatedHours" className="text-xs text-muted-foreground">Estimated duration</Label>
                    <Input
                      id="estimatedHours"
                ...
                <div className="space-y-3">
                  <div className="flex items-center justify-between">
                    <p className="text-[11px] text-muted-foreground uppercase tracking-wider">Milestones</p>
                    <Button type="button" variant="ghost" size="sm" className="h-7 text-xs gap-1 text-muted-foreground" onClick={handleAddMilestone}>
                  ...
                  <div className="space-y-2">
                    {milestones.map((m, idx) => (
                      <div key={idx} className="p-3 rounded-lg border border-border/40 bg-card/50 space-y-3">
                        <div className="flex items-center justify-between">
                          <div className="flex items-center gap-2">
                            <GripVertical className="h-3.5 w-3.5 text-muted-foreground/30" />
                            <span className="text-[11px] font-medium text-muted-foreground">Milestone {idx + 1}</span>
                          </div>
                          <Button
                            type="button"
                            variant="ghost"
                  ...
                  <div className="flex items-center justify-between p-3 rounded-lg border border-primary/20 bg-primary/5">
                    ...
                    <span className="text-sm font-semibold text-primary">${totalAmount.toLocaleString()}</span>
                  </div>

                  <div className="space-y-1.5">
                    <Label htmlFor="totalDuration" className="text-xs text-muted-foreground">Total estimated duration</Label>
                    <Input
                      id="totalDuration"
                ...
                <div className="space-y-1.5">
                  ...
                  <div className="flex gap-2 flex-wrap">
                    {(["full-time", "part-time", "weekends"] as const).map(opt => (
                      ...
                      >
                        {opt === "full-time" ? "Full-time" : opt === "part-time" ? "Part-time" : "Weekends only"}
                      </button>
                    ))}
                  </div>
                </div>
                <div className="space-y-1.5">
                  <Label htmlFor="startDate" className="text-xs text-muted-foreground">Earliest start date</Label>
                  <Input
                    id="startDate"
                    type="date"
                    value={startDate}
            ...
            <section className="space-y-4">
              ...
              <div className="grid sm:grid-cols-2 gap-4">
                ...
                <div className="space-y-1.5">
                  <Label htmlFor="portfolio" className="text-xs text-muted-foreground">Portfolio / website</Label>
                  <div className="relative">
              ...
              <div className="space-y-1.5">
                <Label htmlFor="resume" className="text-xs text-muted-foreground">Resume (PDF, max 5MB)</Label>
                <div className="relative">
                  <Input
                    id="resume"
                    type="file"
