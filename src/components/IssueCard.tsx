...
function getLabelStyles(label: string) {
  ...
  if (l.includes("feature") || l.includes("enhancement")) return "bg-primary/10 text-primary border border-primary/20";
  ...
  return (
    ...
    >
      <div className="flex h-full flex-col">
        ...
        <h4 className="line-clamp-2 text-sm font-medium text-foreground mb-2">#{id}: {title}</h4>

        {/* Bottom: labels */}
        <div className="mt-auto flex items-center gap-1.5 flex-wrap">
          {labels.slice(0, 3).map((label) => (
            <Badge key={label} variant="outline" className={`h-[22px] px-2 text-[10px] ${getLabelStyles(label)}`}>
              <span className="truncate">{label}</span>
            </Badge>
          ))}
          {labels.length > 3 && (
            <Badge variant="outline" className="h-[22px] px-2 text-[10px] bg-muted/50 text-muted-foreground border border-border">+{labels.length - 3}</Badge>
          )}
        </div>
      </div>
    </Card>
