...
export const IssueRow = forwardRef<HTMLButtonElement, { issue: UnifiedIssue; onClick?: () => void } & React.ButtonHTMLAttributes<HTMLButtonElement>>(
  ({ issue, onClick, ...props }, ref) => (
    ...
    >
      <div className="flex items-center gap-2 sm:gap-3 w-full sm:w-auto sm:flex-1 min-w-0">
        ...
        <span className="flex-1 truncate text-foreground group-hover:text-primary transition-colors">{issue.title}</span>
      </div>
