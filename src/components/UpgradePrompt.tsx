...
const UpgradePrompt = ({ plan, feature }: UpgradePromptProps) => {
  ...
  return (
    <Card className="p-8 text-center border-primary/20 bg-card">
      ...
      <h3 className="text-xl font-semibold mb-2">Upgrade to {PLAN_LABELS[plan]}</h3>
      <p className="text-muted-foreground mb-6 max-w-md mx-auto">
        {feature.charAt(0).toUpperCase() + feature.slice(1)} requires the{' '}
        <span className="text-primary font-medium">{PLAN_LABELS[plan]}</span> plan. Upgrade to
        unlock this and more features.
      </p>
