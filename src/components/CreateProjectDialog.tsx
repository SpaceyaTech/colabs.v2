...
import {
  Plus, X, Upload, Globe, Lock, Check, ChevronRight, ChevronLeft,
} from "lucide-react";
...
function getInitialForm(project?: EditableProject | null) {
  ...
  return {
    name: "", description: "", logo: null as File | null, technologies: [] as string[],
    githubUrl: "", figmaUrl: "", slackUrl: "", discordUrl: "", linearUrl: "", otherToolUrl: "",
    projectType: "open-source", visibility: "public",
    isPaid: false, compensationType: "fixed", budget: "", currency: "USD",
    teamSize: "1-3", experienceLevel: "intermediate", duration: "1-3months",
    allowApplications: true, requiresApproval: true, inviteEmails: [] as string[],
    category: "", industry: "", launchReadiness: "concept",
  };
}

export const CreateProjectDialog: React.FC<CreateProjectDialogProps> = ({
  open, onOpenChange, editProject, onSaved,
}) => {
  ...
  const handleSubmit = async () => {
    ...
    if (!formData.name.trim() || !formData.description.trim()) {
      toast.error("Name and description are required"); setStep(0); return;
    }
    ...
    try {
      ...
      if (formData.logo) {
        ...
        if (!allowedExtensions.includes(fileExt)) {
          toast.error("Invalid file type"); setIsSubmitting(false); return;
        }
        const fileName = `${user.id}-${Date.now()}.${fileExt}`;
        const { error: upErr } = await supabase.storage.from('project-logos').upload(fileName, formData.logo);
      ...
      if (isEditMode && editProject) {
        ...
        const { error } = await supabase.from('projects').insert({ ...payload, creator_id: user.id }).select().single();
        if (error) throw new Error(error.message);
            ...
            <>
                  ...
                  <div className="flex gap-2">
                    ...
                    {formData.logo && (
                      <Button type="button" variant="ghost" size="sm" className="h-7 text-xs gap-1.5"
                        onClick={() => { set("logo", null); setLogoPreview(editProject?.logo_url || null); }}>
              ...
              <div className="space-y-1.5">
                ...
                <div className="flex gap-2">
                  ...
                  />
                  <Button onClick={addTechnology} variant="outline" size="sm" className="h-8 w-8 p-0 shrink-0">
                  ...
                  <div className="flex flex-wrap gap-1.5 pt-1">
                    {formData.technologies.map((tech) => (
                      <Badge key={tech} variant="secondary" className="text-[10px] gap-1 pr-1">
                        {tech}
                        <X className="h-2.5 w-2.5 cursor-pointer" onClick={() => set("technologies", formData.technologies.filter(t => t !== tech))} />
                      </Badge>
                    ))}
                  </div>
                )}
              ...
              <div className="grid grid-cols-2 gap-3">
                <div className="space-y-1.5">
                  ...
                  <Select value={formData.category} onValueChange={(v) => set("category", v)}>
                    ...
                    <SelectContent>
                      {["web-app", "mobile-app", "desktop-app", "api", "library", "tool", "game", "other"].map(v => (
                        <SelectItem key={v} value={v} className="text-xs">{v.replace("-", " ")}</SelectItem>
                      ))}
                    </SelectContent>
                  </Select>
                </div>
                <div className="space-y-1.5">
                  ...
                  <Select value={formData.industry} onValueChange={(v) => set("industry", v)}>
                    <SelectTrigger className="h-8 text-xs"><SelectValue placeholder="Select" /></SelectTrigger>
                    <SelectContent>
                      {["fintech", "healthcare", "education", "ecommerce", "social", "productivity", "entertainment", "other"].map(v => (
          ...
          {step === 1 && (
            <>
              ...
              <div className="space-y-2">
                ...
                <RadioGroup value={formData.projectType} onValueChange={(v) => set("projectType", v)} className="space-y-1.5">
                  {[
                    { value: "open-source", label: "Open Source", desc: "Public repo, community contributions", icon: Globe },
                    { value: "private", label: "Private", desc: "Closed source, invite-only", icon: Lock },
                    { value: "hybrid", label: "Hybrid", desc: "Mix of open and closed components", icon: Globe },
                  ].map(opt => (
              ...
              <div className="grid grid-cols-3 gap-3">
                  ...
                  <Select value={formData.teamSize} onValueChange={(v) => set("teamSize", v)}>
                    ...
                    <SelectContent>
                      {["1-3", "4-6", "7-10", "10+"].map(v => (
                        <SelectItem key={v} value={v} className="text-xs">{v} people</SelectItem>
                      ))}
                    </SelectContent>
                  </Select>
                </div>
                <div className="space-y-1.5">
                  ...
                  <Select value={formData.experienceLevel} onValueChange={(v) => set("experienceLevel", v)}>
                    <SelectTrigger className="h-8 text-xs"><SelectValue /></SelectTrigger>
                    <SelectContent>
                ...
                <div className="space-y-1.5">
                  ...
                  <Select value={formData.duration} onValueChange={(v) => set("duration", v)}>
                    <SelectTrigger className="h-8 text-xs"><SelectValue /></SelectTrigger>
                    <SelectContent>
                      {[["1week","1 week"],["1month","1 month"],["1-3months","1-3 months"],["3-6months","3-6 months"],["6months+","6+ months"]].map(([v,l]) => (
              ...
              <div className="space-y-3">
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <p className="text-[10px] text-muted-foreground">Contributors will be compensated</p>
                  </div>
                ...
                {formData.isPaid && (
                  <div className="grid grid-cols-3 gap-3">
                    <div className="space-y-1.5">
                      ...
                      <Select value={formData.compensationType} onValueChange={(v) => set("compensationType", v)}>
                        <SelectTrigger className="h-8 text-xs"><SelectValue /></SelectTrigger>
                        <SelectContent>
                    ...
                    <div className="space-y-1.5">
                      ...
                      <Input value={formData.budget} onChange={(e) => set("budget", e.target.value)} placeholder="5000" className="h-8 text-xs" />
                    </div>
                    <div className="space-y-1.5">
            ...
            <>
              ...
              <div className="space-y-3">
                <div>
                  ...
                  <p className="text-[10px] text-muted-foreground">Add links to your project's tools</p>
                </div>
                <div className="grid grid-cols-2 gap-3">
                  {([
          ...
          {step === 3 && (
            <>
              ...
              <div className="space-y-2">
                ...
                <RadioGroup value={formData.visibility} onValueChange={(v) => set("visibility", v)} className="space-y-1.5">
                  {[
                    ...
                    { value: "invite-only", label: "Invite Only", desc: "Requires invitation to join" },
                  ].map(opt => (
                    <label key={opt.value} className={cn(
              ...
              <div className="space-y-3">
                <div className="flex items-center justify-between">
                  ...
                  <Switch checked={formData.allowApplications} onCheckedChange={(v) => set("allowApplications", v)} />
                </div>
                <div className="flex items-center justify-between">
                  <div>
                    <Label className="text-xs font-medium">Require Approval</Label>
                    <p className="text-[10px] text-muted-foreground">Manually approve new contributors</p>
                  </div>
                  <Switch checked={formData.requiresApproval} onCheckedChange={(v) => set("requiresApproval", v)} />
                </div>
              </div>
                ...
                <div className="flex gap-2">
                  <Input
                    ...
                    onKeyDown={(e) => {
                      if (e.key === "Enter") {
                        e.preventDefault();
                        if (currentEmail.trim() && !formData.inviteEmails.includes(currentEmail.trim())) {
                    ...
                    className="h-8 text-xs"
                  />
                  <Button variant="outline" size="sm" className="h-8 w-8 p-0 shrink-0" onClick={() => {
                    if (currentEmail.trim() && !formData.inviteEmails.includes(currentEmail.trim())) {
                      set("inviteEmails", [...formData.inviteEmails, currentEmail.trim()]);
                      setCurrentEmail("");
                    }
                  ...
                  <div className="flex flex-wrap gap-1.5 pt-1">
                    {formData.inviteEmails.map(email => (
                      <Badge key={email} variant="secondary" className="text-[10px] gap-1 pr-1">
                        {email}
                        <X className="h-2.5 w-2.5 cursor-pointer" onClick={() => set("inviteEmails", formData.inviteEmails.filter(e => e !== email))} />
                      </Badge>
                    ))}
                  </div>
                )}
