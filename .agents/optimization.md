...
For functions that return large datasets, return a streaming response:
...
The GitHub API allows 5,000 authenticated requests/hour. The `github-issues` function makes one request per collaboration-enabled repo per invocation.

Caching strategy to stay within limits:

- Store the `ETag` header from GitHub API responses in a metadata column on `github_repositories`
- On subsequent requests, send `If-None-Match: <etag>` — GitHub returns `304 Not Modified` and does not count against the rate limit
...
if (response.status === 304) {
  // Return cached data from DB — don't parse body
  return cachedIssues;
}

// Update cached etag in github_repositories
